...
const cancun = {
  nombre: 'Cancún — Viva Maya All Inclusive',
  categoria: 'internacional',
  descripcion: 'Paquete internacional a Cancún con vuelos LATAM desde Salta, 7 noches en Viva Maya by Wyndham All Inclusive en Playa del Carmen, traslados y asistencia al viajero.',
  precio: 0,
  precio_desde: 2400,
  origen_ciudad: 'Salta',
  destino_ciudad: 'Cancún / Playa del Carmen',
  duracion_dias: 8,
  duracion_noches: 7,
  incluye: [
  ...
  alojamiento: {
    nombre: 'Viva Maya by Wyndham',
    descripcion: 'Resort All Inclusive en Playa del Carmen, a pocos metros de la playa con múltiples piscinas, restaurantes y entretenimiento.',
    estrellas: 5,
  },
  itinerario: [
    { dia: 1, titulo: 'Llegada a Cancún', descripcion: 'Vuelo desde Salta. Llegada al aeropuerto de Cancún a las 14:20 hs. Traslado al hotel Viva Maya by Wyndham en Playa del Carmen. Check-in y tarde libre.', actividades: ['Vuelo Salta → Cancún', 'Traslado aeropuerto → hotel', 'Check-in y bienvenida All Inclusive'] },
    ...
    { dia: 5, titulo: 'Chichén Itzá (optativo)', descripcion: 'Posibilidad de visita a Chichén Itzá, una de las 7 maravillas del mundo moderno (excursión optativa no incluida).', actividades: ['Chichén Itzá (optativo)', 'Cenote Ik Kil (optativo)', 'Valladolid (optativo)'] },
    ...
    { dia: 7, titulo: 'Último día en Playa del Carmen', descripcion: 'Mañana libre hasta el checkout. Tarde de compras en la Quinta Avenida o relax en la playa.', actividades: ['Checkout del hotel', 'Tarde libre en Playa del Carmen'] },
...
const { data, error } = await supabase.from('productos').insert(cancun).select().single()

if (error) {
  console.error('Error:', error.message)
  process.exit(1)
}
console.log('✅ Paquete Cancún insertado con ID:', data.id)
